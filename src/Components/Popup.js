...
import { createMovie,deleteMovie } from "../actions"
...
<div className="popupWindow">
    ...
    printErrorMessege=()=>{
        if(this.state.errorMsg!==""){
            return <div className="error-messege">{this.state.errorMsg}</div>
...
return null
    ...
    validateForm = (movieObj) => {
        ...
        if(this.checkUniqueTitle(movieObj.Title))return {status:false,msg:"Title is already Taken"}
        return {status:true}
    }
    checkUniqueTitle=(newTitleStr)=>{
    ...
    getTextFields = () => {
        return {
            Title: this.convertToValidString(document.querySelector('.field-Title').value),
